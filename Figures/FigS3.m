...
param_list = cellfun(@(x) strrep(x,'_','\_'),param_list,'UniformOutput',false);
...
label = label_raw(idx);
select_idx = abs(data)>=1;
barh(1:sum(select_idx),data(select_idx),0.4,'FaceColor',[33,102,172]/255,'EdgeColor',[33,102,172]/255,'LineWidth',0.5);
xlim([-100 100]);
set(gca,'YTick',1:1:sum(select_idx));
...
label = label_raw(idx);
select_idx = abs(data)>=1;
barh(1:sum(select_idx),data(select_idx),0.4,'FaceColor',[178,24,43]/255,'EdgeColor',[178,24,43]/255,'LineWidth',0.5);
xlim([-100 100]);
set(gca,'YTick',1:1:sum(select_idx));
set(gca,'YTickLabel',label(select_idx));
set(gca,'FontSize',7,'FontName','Helvetica');
...
set(gcf,'position',[450 50 550 85]);
...
xlabel('Relative change in growth (%)','FontSize',7,'FontName','Helvetica','Color','k');
title('2-fold increase','FontSize',7,'FontName','Helvetica','Color','k');
set(gcf,'position',[450 550 550 210]);
